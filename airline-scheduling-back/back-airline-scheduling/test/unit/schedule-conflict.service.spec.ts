...
describe('ScheduleConflictService (unit)', () => {
  ...
  beforeEach(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(Flight), useValue: repoMock },
        { provide: getRepositoryToken(Aircraft), useValue: repoMock },
        { provide: getRepositoryToken(MaintenanceSlot), useValue: repoMock },
        { provide: getRepositoryToken(CrewAssignment), useValue: repoMock },
      ],
    }).compile();
  ...
  it('bloque un vol dont l’arrivée est antérieure au départ', async () => {
  ...
  it('signale un avion non affecté sans bloquer la saisie', async () => {
    const result = await service.validateCandidate({
      numeroVol: 'MD045',
      aeroportDepart: 'TNR',
      aeroportArrivee: 'NOS',
